...
const StreaksScreen = () => {
  ...
  useEffect(() => {
    if (user) {
      ...
      const completinsSubscription = client.subscribe(
        completionsChannel,
        (response: RealTimeResponse) => {
          if (
            response.events.includes(
  ...
  const getStreakData = (habitId: string): StreakData => {
    ...
    completedHabits.forEach((c) => {
      ...
      if (lastData) {
        const diff =
          (date.getTime() - lastData.getTime()) / (1000 * 60 * 60 * 24);
  ...
  const rankedHabit = habitStreaks.sort((a, b) => b.bestStreak - a.bestStreak);
    ...
    <View style={styles.container}>
      ...
      {rankedHabit.length > 0 && (
        <View style={styles.rankingContainer}>
          ...
          {rankedHabit.slice(0, 3).map((item, key) => (
            <View key={key} style={styles.rankingRow}>
              <View style={[styles.rankingBadge, badgeStyles[key]]}>
                <Text style={styles.rankingBadgeNumber}> {key + 1}</Text>
              </View>
              <Text style={styles.rankingBadgeTitle}>{item.habit.title}</Text>
              <Text style={styles.rankingBadgeStreak}>{item.bestStreak}</Text>
            </View>
          ))}
        </View>
      )}
      ...
      {habits.length === 0 ? (
        <View>
          <Text>No Habit yet. Add your first Habbit!</Text>
        </View>
      ) : (
        <ScrollView
        ...
        >
          {rankedHabit.map(({ habit, streak, bestStreak, total }, key) => (
            <Card
              key={key}
